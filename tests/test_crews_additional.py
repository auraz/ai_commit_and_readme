...
class TestEnrichmentCrew:
    ...
    def test_execute_no_code_blocks(self):
        ...
        with patch.object(crew, '_create_crew') as mock_create_crew:
            ...
            needs_update, content = crew._execute(
                diff="test diff",
                doc_content="old",
                doc_type="README",
                file_path="README.md"
...
class TestPipelineCrew:
    ...
    def test_execute_debug_diff_preview(self, monkeypatch, caplog):
        ...
        with patch.object(crew, '_get_git_diff', return_value=long_diff):
            with patch.object(crew, '_process_documents', return_value={"suggestions": {}, "selected_articles": []}):
                with patch.object(crew, '_write_outputs'):
                    with caplog.at_level("DEBUG"):
                        crew._execute()
