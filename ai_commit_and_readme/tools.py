...
def get_ai_response(prompt: str, ctx: Optional[CtxDict] = None, json_response: bool = False, temperature: float = 1.0) -> Any:
...
def get_ai_evaluation(prompt: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "max_score": 100,
            "grade": "Improve",
            "summary": f"Evaluation failed due to an error: {e!s}",
            "top_recommendations": ["Check API key and network connection.", "Try again later."],
        }


def format_evaluation_results(evaluation: Dict[str, Any], title: str, categories: Dict[str, int]) -> str:
...
def evaluate_with_ai(file_path: str, prompt_filename: str, format_vars: Dict[str, str], categories: Dict[str, int], report_title: str) -> Tuple[int, str]:
    ...
    try:
        # Determine if this is an eval prompt from evals directory
        ...
        # Format the template with the content variables
        ...
        return 0, f"Error in prompt template: {e}"
    except Exception as e:
...
def load_composite_template(prompt_filename: str, format_vars: Dict[str, str]) -> str:
    ...
    # Just load the template file directly
    with open(prompt_path, encoding="utf-8") as f:
        template = f.read()
